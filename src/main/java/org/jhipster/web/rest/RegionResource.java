...
public class RegionResource {
    ...
    public ResponseEntity<List<Region>> getAllRegions(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("lake-is-null".equals(filter)) {
            log.debug("REST request to get all Regions where lake is null");
            return new ResponseEntity<>(regionService.findAllWhereLakeIsNull(),
                    HttpStatus.OK);
        }
